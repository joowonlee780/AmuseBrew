...
public class Playercontroller2_donghee : MonoBehaviour
{
    ...
    {
        // 지구력이 0보다 크면 달릴 수 있게, 0 아래면 RunningCancle을 호출하고 리턴      
        if (endurance <= 0)
        {
            RunningCancle();
            return;
    ...
    {
        /*// 지구력--; + 추가로 딜레이 주기, 달리고 있을 때 1초마다 지구력이 얼만큼 줄어든다는 식으로
         * 
         * if(Invoke(Walk)가 실행 중이라면)CancleInvoke("Walk");
         * if(Invoke(Run)이 실행 중이지 않다면)InvokeRepeating("Run", 0f, 1f);
         * 
         * if(지구력이 0 아래로 떨어지면)
         */

        if (IsInvoking("IsWalking")) CancelInvoke("IsWalking");
        if (!IsInvoking("IsRunning")) InvokeRepeating("IsRunning", 0f, 1f);
    ...
    {
        /* // 지구력++; + 추가로 딜레이 주기, 달리고 있지 않을 때 2초마다 지구력이 얼만큼 채워진다는 식으로
         * 
         * if(Invoke(Run)이 실행 중이라면)CancleInvoke("Run");
         * if(Invoke(Walk)가 실행 중이지 않다면)InvokeRepeating("Walk", 2f, 2f);
         * 
         */

        if (IsInvoking("IsRunning")) CancelInvoke("IsRunning");
        if (!IsInvoking("IsWalking")) InvokeRepeating("IsWalking", 2f, 2f);
    ...
    {
        endurance -= 1f;
        enduranceImg.fillAmount = endurance / 5f;
